...
for pair in elfGroups:
    #compare the lows and highs of the elves in each group to determine which are redundant
    if int(pair[0][0]) >= int(pair[1][0]) and int(pair[0][1]) <= int(pair[1][1]):
        #elf1 section contained in section of elf2
        total += 1
    ...
    #use elif here to make sure a group doesn't get counted twice if both ranges are exactly equal
        ...
        #elf2 section contained in section of elf1
        total += 1
